...
def outbound_analysis(log):
    ...
    if dest_port not in [80, 443, 53, 5353, 1900]:
        ...
        if now - last > COOLDOWN:
            print(f"[WARNING] Unusual outbound connection attempt: {src_ip} → {dest_ip}:{dest_port}")
            ...
            send_alert(
                ...
                alert_type="IP",
                entity=dest_ip,
                reason=f"Outbound connection to unusual port {dest_port}"
            )
    ...
    if len(outbound_events[src_ip]) >= CONN_THRESHOLD:
        ...
        send_alert(
            ...
            alert_type="IP",
            entity=src_ip,
            reason=f"{CONN_THRESHOLD} outbound connections in {TIME_WINDOW}s"
        )
...
def dns_analysis(log):
    ...
    if len(dns_events[src_ip]) >= DNS_THRESHOLD:
        ...
        send_alert(
            ...
            alert_type="IP",
            entity=src_ip,
            reason=f"{DNS_THRESHOLD} DNS queries in {DNS_WINDOW}s"
        )
...
def https_analysis(log):
    ...
    if len(https_sni_events[src_ip]) >= HTTPS_SNI_THRESHOLD:
        ...
        print(f"[WARNING] Excessive HTTPS domains from {src_ip}")
